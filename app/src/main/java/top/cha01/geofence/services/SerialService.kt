package top.cha01.geofence.services
...
import top.cha01.geofence.Constants
import top.cha01.geofence.R
import top.cha01.geofence.libs.UsbSerial.SerialSocket
...
class SerialService : Service(), SerialListener {
    ...
    private val queue1 =
        ArrayDeque<SerialService.QueueItem>()
    private val queue2 =
        ArrayDeque<SerialService.QueueItem>()
    private val lastRead = SerialService.QueueItem(QueueType.Read)
        ...
        if (connected) {
            synchronized(this) {
                if (listener != null) {
                    ...
                    queue2.add(SerialService.QueueItem(QueueType.Connect))
                }
            }
        }
    }
        ...
        if (connected) {
            synchronized(this) {
                if (listener != null) {
                    ...
                    queue2.add(SerialService.QueueItem(QueueType.ConnectError, e))
                    disconnect()
                }
            }
        }
            ...
            synchronized(this) {
                if (listener != null) {
                    ...
                    if (queue2.isEmpty() || queue2.last.type != QueueType.Read) queue2.add(
                        SerialService.QueueItem(QueueType.Read)
                    )
        ...
        if (connected) {
            synchronized(this) {
                if (listener != null) {
                    ...
                    queue2.add(SerialService.QueueItem(QueueType.IoError, e))
                    disconnect()
                }
            }
        }
